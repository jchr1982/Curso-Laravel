...
if (!function_exists('canUser')) {
    function can($permiso, $redirect = true)
    {
        if (session()->get('rol_nombre') == 'administrador') {
            ...
            /* utilizamos la cache de laravel para no tener
            que consultar la base de datos cada vez que
            se requiere obtener los permisos */
            $permisos = cache()->tags('Permiso')->rememberForever("Permiso.rolid.$rolId", function () {
                // consulto la base de datos para buscar los permisos 
                // con todos sus roles
                return Permiso::whereHas('roles', function ($query) {
                    $query->where('rol_id', session()->get('rol_id'));
                })->get()->pluck('slug')->toArray();
            });
            // pregunto si este permiso que me llega existe dentro del 
            // array que he creado
